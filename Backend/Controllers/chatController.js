...
exports.getConversations = async (req,res)=>{
    ...
    try{
        let conversations = await Conversation.find({participants:userId}).populate({path:"lastMessage",populate:{path:"sender receiver",select:"userName profilePicture"}}).sort({updatedAt:-1});
        return res.status(201).json({"conversation get successfully":conversations});  
    }
    catch(err){
        console.error(err);
        return res.status(500).json({message:"Internal server error"});
